...
public class RomanConverter {
	...
	public static int getNumberFromRoman(String a) throws IllegalArgumentException {
		if (!VALIDATION_RE.matcher(a).matches()) {
			throw new IllegalArgumentException("Numéral romain invalide !");
		}
		int result    = 0;
		int prevValue = 0;
		for (int i = a.length() - 1; i >= 0; i--) {
			int I = i;
			int currValue = SYMBOLS.stream()
					.filter(symbol -> symbol.getRoman().equals(a.substring(I, I + 1)))
					.findFirst()
					.map(RomanNumber::getValue)
					.orElse(0);
